...
// B-min versioning (introduced with rule 9, 2026-06-27). This previously-
// unversioned prompt now carries a version constant + SHA256 prompt hash
// (first 8 hex), mirroring the domain-detector pattern in server.js. Both are
// logged into /format-draft generation_metadata on every call so prompt drift
// is catchable at log inspection. Bump VERSION whenever the addendum string
// changes; the HASH recomputes automatically from the string.
const { createHash } = require('crypto');

const FORMAT_DRAFT_SLOT_ADDENDUM_VERSION = 'v2';

const FORMAT_DRAFT_SLOT_ADDENDUM_HASH = createHash('sha256')
  .update(FORMAT_DRAFT_SLOT_ADDENDUM)
  .digest('hex')
  .substring(0, 8);
