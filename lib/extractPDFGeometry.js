...
function buildLineRuns(glyphs) {
  ...
  for (const g of gs) {
    ...
    const key = `${g.font || ''}|${g.sizeHp}|${g.bold ? 1 : 0}|${g.italic ? 1 : 0}|${g.color || ''}`;
    if (cur && cur._key === key) {
      cur.text += piece;
    } else {
      cur = {
        ...
        color: g.color || undefined, // hex (no #); undefined = default black. Phase E colour capture.
        _key: key,
      };
      runs.push(cur);
    }
...
// Glyph fill colour → run.color; cell/region fill colour → cell.shading, both
// rendered by buildReportDocxV2 (which already supports them). HARD GUARDRAIL:
// this is a read-only, render-only channel. It NEVER feeds detectTablesFromSegments,
// eligible(), or the visible-rule gate. extractPageGraphics keeps tracking `fill`
// exactly as before (same setFillRGBColor / setFillColorN handling) and only
// READS it at text-show ops; the fills surfaced for shading are exactly the ones
// eligible() already kept — so capturing a colour cannot change whether a fill
// votes in table inference.
...
// Perceptual luminance (Rec. 601), 0..1.
function colorLuminance(c) {
  return 0.299 * c[0] + 0.587 * c[1] + 0.114 * c[2];
}
const NEAR_BLACK_MAX = 0.06;     // darker than this on every channel → "default black" (carry no colour)
const MIN_TEXT_CONTRAST = 0.20;  // luminance gap below which text is treated as invisible on its bg

function isNearBlack(c) {
  return !!c && c[0] <= NEAR_BLACK_MAX && c[1] <= NEAR_BLACK_MAX && c[2] <= NEAR_BLACK_MAX;
}

// WHITE-ON-COLOUR SAFETY (content-safety, not cosmetics). Given a captured run
// colour (hex, or null = default black) and the background it will actually
// render on (rgb — a cell's shading, else the white page), return the colour to
// emit: keep it when it has enough contrast, else fall back to the visible
// extreme — black on a light bg, white on a dark bg. Guarantees no glyph renders
// invisibly (the white-on-white hard fail). Returns undefined to mean "default
// black" (emit no explicit colour).
function safeRunColor(colorHex, bgRgb) {
  const bg = bgRgb || [1, 1, 1];
  const c = colorHex ? hexToRgb(colorHex) : [0, 0, 0];
  if (!c) return colorHex; // unparseable → leave as-is
  if (Math.abs(colorLuminance(c) - colorLuminance(bg)) >= MIN_TEXT_CONTRAST) return colorHex;
  return colorLuminance(bg) > 0.5 ? undefined : 'FFFFFF';
}

// Assign each glyph a fill colour from the text-show colour records captured in
// the operator-list walk ({yTop, x, color(rgb)} at each run origin). A glyph
// takes the colour of the nearest-baseline record starting at or before it —
// handling multi-colour lines, degrading to per-line when a line is one colour.
// Near-black stays uncoloured (renders as default black, fewer run splits).
function assignGlyphColors(glyphs, textColors) {
  if (!textColors || textColors.length === 0) return;
  const byY = new Map();
  for (const t of textColors) {
    const k = Math.round(t.yTop);
    if (!byY.has(k)) byY.set(k, []);
    byY.get(k).push(t);
  }
  const nearestOnLine = (g, requireBefore) => {
    let best = null, bestX = requireBefore ? -Infinity : Infinity;
    for (let dk = -2; dk <= 2; dk++) {
      const bucket = byY.get(Math.round(g.baseTop) + dk);
      if (!bucket) continue;
      for (const t of bucket) {
        if (Math.abs(t.yTop - g.baseTop) > 3) continue;
        if (requireBefore) { if (t.x <= g.xLeft + 2 && t.x > bestX) { best = t; bestX = t.x; } }
        else if (t.x < bestX) { best = t; bestX = t.x; }
      }
    }
    return best;
  };
  for (const g of glyphs) {
    const best = nearestOnLine(g, true) || nearestOnLine(g, false);
    if (best && best.color && !isNearBlack(best.color)) g.color = rgbToHex(best.color);
  }
}

// Smallest enclosing visible fill whose rect contains a point (top-down pt) → its
// hex colour, for cell shading. Skips ~white fills (no visible tint). `fills` is
// the eligible coloured-fill list surfaced by extractPageGraphics.
function cellShadingFor(cx, cy, fills) {
  let best = null, bestArea = Infinity;
  for (const f of fills || []) {
    const r = f.rect;
    if (!r || !f.fill) continue;
    if (cx < r.x0 - 0.5 || cx > r.x1 + 0.5 || cy < r.y0 - 0.5 || cy > r.y1 + 0.5) continue;
    if (f.fill[0] > 0.96 && f.fill[1] > 0.96 && f.fill[2] > 0.96) continue; // ~white → no tint
    const area = r.width * r.height;
    if (area < bestArea) { best = f; bestArea = area; }
  }
  return best ? rgbToHex(best.fill) : undefined;
}

// Final colour pass over the assembled structure: enforce WHITE-ON-COLOUR SAFETY
// using each run's TRUE output background (a cell's shading, else the white page).
// Runs in a shaded cell are judged against that shading; everything else against
// white. A run that would vanish is flipped to black/white as appropriate.
function applyColorContrastSafety(structure) {
  const WHITE = [1, 1, 1];
  const fixRuns = (block, bg) => {
    for (const r of block.runs || []) {
      const safe = safeRunColor(r.color, bg);
      if (safe) r.color = safe; else delete r.color;
    }
  };
  const walk = (blocks, bg) => {
    for (const b of blocks || []) {
      if (b.type === 'paragraph') fixRuns(b, bg);
      else if (b.type === 'table') {
        for (const row of b.rows || []) {
          for (const cell of row.cells || []) {
            const cbg = cell.shading ? (hexToRgb(cell.shading) || WHITE) : WHITE;
            walk(cell.content, cbg);
          }
        }
      }
    }
  };
  walk(structure, WHITE);
}
...
// A VISIBLE decorative fill NEAR a table — a full-width section band, a label
// strip, or a row of chips above/below the grid — emits rule segments that
// cluster into the same horizontal-rule band as the real grid, so
// detectTablesFromSegments absorbs it as extra leading / trailing / detached
// rows (FAB_hard_colour §2: "SECTION 2" band + "Priority goal areas" + chips
// were swallowed as table rows 0/1/8/9 around the real Receptive…Feeding grid).
//
// The structural truth: a real grid row is crossed by ≥1 INTERIOR vertical rule
// (a column rule strictly between the table's left and right edges). A full-width
// decorative band is one cell — no interior rule crosses it. So the table's true
// extent is the MAXIMAL CONTIGUOUS run of grid rows; rows outside it are peeled
// back to paragraphs (left UNMARKED in `used` by the caller, so they flow to
// linesToParagraphs exactly like the E-15 / E-24b rejection paths).
//
// This reads ONLY vertical-rule geometry (x position + y span) — the same
// segments detectTablesFromSegments already consumed. It never inspects fill
// colour, so it is orthogonal to E-24/E-24b: a fill's colour cannot change
// whether its row is judged structural.
const INTERIOR_X_TOL = 3.0; // pt — a rule within this of an outer edge is an edge, not interior

function rowHasInteriorRule(y0, y1, Vs, left, right) {
  const midY = (y0 + y1) / 2;
  for (const v of Vs) {
    if (v.x > left + INTERIOR_X_TOL && v.x < right - INTERIOR_X_TOL &&
        v.y1 <= midY + 1 && v.y2 >= midY - 1) return true;
  }
  return false;
}

// Return a copy of `desc` whose rowYs cover only the maximal contiguous run of
// grid rows. Returns the original desc when nothing needs trimming, or null when
// NO row is a grid row (caller drops it → all its lines flow to paragraphs).
function trimTableToGridCore(desc, segs) {
  const { rowYs, left, right } = desc;
  const Vs = (segs || []).filter((s) => s.vertical);
  const nRows = rowYs.length - 1;
  if (nRows < 1) return null;
  let bestStart = -1, bestLen = 0, curStart = -1, curLen = 0;
  for (let r = 0; r < nRows; r++) {
    if (rowHasInteriorRule(rowYs[r], rowYs[r + 1], Vs, left, right)) {
      if (curStart < 0) curStart = r;
      curLen++;
  ...
  if (bestLen === 0) return null; // no structural row at all → not a real table
  if (bestStart === 0 && bestLen === nRows) return desc; // already all grid rows
  const newRowYs = rowYs.slice(bestStart, bestStart + bestLen + 1);
  return Object.assign({}, desc, {
    rowYs: newRowYs,
    top: newRowYs[0],
    bottom: newRowYs[newRowYs.length - 1],
  });
}
...
function buildTableBlock(desc, regionLines, fills) {
  ...
  for (let r = 0; r < nRows; r++) {
    ...
    for (let c = 0; c < nCols; c++) {
      ...
      const cx = (colXs[c] + colXs[c + 1]) / 2;
      const cy = (rowYs[r] + rowYs[r + 1]) / 2;
      const shading = cellShadingFor(cx, cy, fills); // RENDER-ONLY: tinted-cell fill
      const cell = {
        content,
...
async function extractPageGraphics(page, pageH, pageW, OPS) {
  ...
  // Phase E colour capture (RENDER-ONLY) — track the text matrix so each text-show
  // op can record the active fill colour at its origin. tm/tlm are NOT part of the
  // q/Q graphics-state stack (they reset at BT), so the stack below is unchanged.
  let tm = [1, 0, 0, 1, 0, 0];
  let tlm = [1, 0, 0, 1, 0, 0];
  let leading = 0;
  const textColors = []; // {yTop, x, color(rgb)} per text-show — consumed only by assignGlyphColors
  const fills = []; // eligible coloured fills {rect, fill} — consumed only by cell shading
  const recordTextColor = () => {
    if (!fill) return; // pattern / unknown text fill → leave glyph at default black
    const [ox, oy] = applyMatrix(matMul(tm, ctm), 0, 0);
    textColors.push({ yTop: pageH - oy, x: ox, color: fill.slice() });
  };
  ...
  for (let i = 0; i < fns.length; i++) {
    ...
    if (fn === OPS.save) {
      ...
      tm = [1, 0, 0, 1, 0, 0]; tlm = tm.slice();
    } else if (fn === OPS.setTextMatrix) {
      tm = (args[i] || [1, 0, 0, 1, 0, 0]).slice(); tlm = tm.slice();
    } else if (fn === OPS.setLeading) {
      leading = (args[i] || [0])[0];
    } else if (fn === OPS.setLeadingMoveText) {
      const a = args[i] || [0, 0]; leading = -a[1]; tlm = matMul([1, 0, 0, 1, a[0], a[1]], tlm); tm = tlm.slice();
    } else if (fn === OPS.moveText) {
      const a = args[i] || [0, 0]; tlm = matMul([1, 0, 0, 1, a[0], a[1]], tlm); tm = tlm.slice();
      ...
      tlm = matMul([1, 0, 0, 1, 0, -leading], tlm); tm = tlm.slice(); recordTextColor();
  ...
  for (const p of paths) {
    if (eligible(p, paths, pageBg, E24_MAX_RULE_FRAC, pageArea)) {
      ...
      // RENDER-ONLY: surface eligible coloured fills for cell shading. Reuses the
      // eligible() verdict above (no new decision) — purely additive to detection.
      if ((p.type === 'f' || p.type === 'fs') && p.fill && p.rect) fills.push({ rect: p.rect, fill: p.fill });
    }
  }
  return { segs, imgs, textColors, fills };
}
...
async function extractPDFGeometry(pdfBuffer) {
  ...
  for (let p = 1; p <= doc.numPages; p++) {
    ...
    const { segs, imgs, textColors, fills } = await extractPageGraphics(page, pageH, pageW, OPS);
    const glyphs = await extractPageGlyphs(page, pageH);
    assignGlyphColors(glyphs, textColors); // Phase E colour capture (render-only channel)
    ...
    for (const d0 of tableDescs) {
      ...
      if (!tableHasVisibleRule(d0, segs)) continue;
      // Decorative-fill over-merge guard: trim to the grid core (drop leading/
      // trailing/detached band rows that no interior column rule crosses). Peeled
      // rows stay UNMARKED in `used` → they flow to paragraphs via freeLines.
      const d = trimTableToGridCore(d0, segs);
      if (!d) continue; // no structural row → not a table; lines become paragraphs
      const regionLines = lines.filter((l) => l.baseTop >= d.top - 2 && l.baseTop <= d.bottom + 2);
      const tbl = buildTableBlock(d, regionLines, fills);
  ...
  applyColorContrastSafety(structure); // WHITE-ON-COLOUR SAFETY: per-run, against true bg
