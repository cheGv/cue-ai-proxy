...
const {
  ...
  VerticalAlign, LevelFormat, TextWrappingType, TabStopType,
  HorizontalPositionRelativeFrom, VerticalPositionRelativeFrom,
} = require('docx');

const EMU_PER_PX = 9525; // docx-js transformation width/height are in px (px*9525 = EMU)
const EMU_PER_DXA = 635; // 914400 EMU/in ÷ 1440 DXA/in
...
// ── runs ─────────────────────────────────────────────────────────────────────
// E-25: map a geometry tab-stop type to docx-js. Left only for now (the column
// detector emits left tab stops); the switch leaves room without a new dep.
function tabTypeOf(val) {
  switch (val) {
    case 'right': return TabStopType.RIGHT;
    case 'center': return TabStopType.CENTER;
    case 'decimal': return TabStopType.DECIMAL;
    default: return TabStopType.LEFT;
  }
}
...
function textRunsFor(runs, textTransform) {
  const out = [];
  for (const r of runs || []) {
    // E-25: a tab marker run → a real <w:tab/> (advances to the paragraph's next
    // tab stop). Carries no text, so textTransform never sees it.
    if (r && r.tab) { out.push(new TextRun({ children: [new Tab()] })); continue; }
...
// Mirror P1 — scale a band image down to fit a max width, preserving aspect
// ratio (never distort, never upscale). Source letterhead bands often exceed the
// A4 printable width by a few pt and would clip at the right edge; this returns a
// media copy whose anchor width/height are scaled together. Returns the media
// unchanged when it already fits.
function fitMediaToWidth(media, maxWidthEmu) {
  const a = media.anchor || {};
  const w = a.width || 0;
  const h = a.height || 0;
  if (!maxWidthEmu || w <= maxWidthEmu || w <= 0) return Object.assign({}, media, { wrap_mode: 'inline' });
  const scale = maxWidthEmu / w;
  return Object.assign({}, media, {
    wrap_mode: 'inline',
    anchor: Object.assign({}, a, { width: Math.round(w * scale), height: Math.round(h * scale) }),
  });
}

// Mirror P1 — a header/footer band paragraph: the recurring letterhead/footer
// image(s) rendered INLINE (never floating — the band's position is fixed by the
// section, and Word repeats the header/footer on every page). Inline avoids the
// reflow drift that floating anchors suffer.
function bandParagraph(mediaList) {
  const runs = [];
  for (const m of mediaList || []) {
    const ir = imageRunFor(Object.assign({}, m, { wrap_mode: 'inline' }));
    if (ir) runs.push(ir);
  }
  if (runs.length === 0) runs.push(new TextRun({ text: '' }));
  return new Paragraph({ children: runs, spacing: { before: 0, after: 0 } });
}

// ── paragraphs ───────────────────────────────────────────────────────────────
function paragraphFor(block, ctx, blockIndex) {
  // Phase D wk3/PT — render-mode-aware. content_fill: slot→inject, static→
  // verbatim, unclassified→blank (drop text, keep images). verbatim: pure mirror.
  const key = blockIndex != null ? `p:${blockIndex}` : null;
  if (ctx.renderMode === 'content_fill' && key) {
    // E-14: multi-field paragraph? If any field_idx-bearing slots exist for
    // this block, render via injectedMultiFieldParagraph (one [label][value]
    // pair per field_idx slot, in source-run order). Field_idx-absent
    // paragraph slots fall through to the existing single-slot path below —
    // backward-compatible for every stored map that pre-dates E-14.
    const fieldSlots = ctx.fieldSlotsByBlock && ctx.fieldSlotsByBlock[blockIndex];
    if (fieldSlots && fieldSlots.length > 0 && ctx.contentMap) {
      return injectedMultiFieldParagraph(block, fieldSlots, ctx);
    }
    const pslot = ctx.slotByLoc[key];
    if (pslot && ctx.contentMap) return injectedParagraph(block, pslot, ctx);
    if (!ctx.staticByLoc[key]) return blankParagraph(block, ctx);
    // static scaffolding → fall through to verbatim render
  }
  ...
  // E-25: left tab stops at borderless-column x's (dxa from the left margin). The
  // run list carries {tab:true} markers between cells; this positions them. Never
  // a table — alignment lives entirely in paragraph tab stops.
  if (Array.isArray(block.tabStops) && block.tabStops.length) {
    opts.tabStops = block.tabStops.map((t) => ({ type: tabTypeOf(t.type), position: t.position }));
  }
  if (block.spacing) {
    opts.spacing = {};
...
function injectedParagraph(block, slot, ctx) {
  ...
  // Preserve any image anchored on this paragraph (e.g. a page logo) even when
  // the paragraph's TEXT is a content slot — images are structural, not clinical.
  for (const img of block.images || []) { const ir = imageRunFor(resolveMedia(img, ctx)); if (ir) children.push(ir); }
...
// E-14: multi-field paragraph injection. A demographic line like "Case Name:
// Asha  Date of report: 19-04-2022" carries N (label, value) pairs in its
// runs[] (alternating bold-label-ending-in-colon + regular-value). The slot
// map decomposes it into N slots with location.field_idx 0..N-1. The renderer
// walks runs[] in pairs, preserves the labels verbatim from source (they ARE
// structural scaffolding), and replaces each value with the matching slot's
// content from contentMap. A pair without a matching slot OR a missing
// contentMap entry → BLANK value (content_fill safety: never leak prior
// client content). Per-run styling (font/size/bold/italic) is preserved.
function injectedMultiFieldParagraph(block, fieldSlots, ctx) {
  ...
  for (let i = 0; i < runs.length; i += 2) {
    ...
    if (labelRun) {
      children.push(new TextRun({
        text: labelRun.text != null ? String(labelRun.text) : '',
        font: labelRun.font || undefined,
        size: typeof labelRun.size === 'number' ? labelRun.size : undefined,
        bold: labelRun.bold || undefined,
        italics: labelRun.italic || undefined,
      }));
    }
    if (valueRun) {
      const matching = fieldSlots[fieldIdx];
      let valueText = '';
      if (matching && ctx.contentMap) {
        const v = ctx.contentMap[matching.slot.slot_id];
        if (v != null) valueText = String(v);
      }
      // Preserve a leading-space prefix from the source value run so the
      // visual gap between label and value matches the template.
      const srcLeading = (valueRun.text || '').match(/^\s+/);
      if (srcLeading && valueText && !/^\s/.test(valueText)) {
        valueText = srcLeading[0] + valueText;
      }
      children.push(new TextRun({
        text: valueText,
        font: valueRun.font || undefined,
        size: typeof valueRun.size === 'number' ? valueRun.size : undefined,
        bold: valueRun.bold || undefined,
        italics: valueRun.italic || undefined,
      }));
      fieldIdx++;
    }
  }
...
// Unclassified paragraph in content_fill mode: emit NO source text (never leak
// the prior client's clinical content) but KEEP structural images (e.g. a logo).
function blankParagraph(block, ctx) {
  const children = [];
  for (const img of block.images || []) { const ir = imageRunFor(resolveMedia(img, ctx)); if (ir) children.push(ir); }
  if (children.length === 0) children.push(new TextRun({ text: '' }));
  const opts = { children };
...
function cellChildren(cell, ctx, blockIndex, r, ci, domainIndex) {
  const key = blockIndex != null ? `${blockIndex}:${r}:${ci}` : null;
  if (ctx.renderMode === 'content_fill') {
    const slot = key ? ctx.slotByLoc[key] : null;
    if (slot && ctx.contentMap) return injectedChildren(cell, slot, ctx, domainIndex);
    // static cell → verbatim (below); unclassified cell → blank (never leak the
    // source template's clinical content).
    if (key && !ctx.staticByLoc[key]) return [new Paragraph({ children: [new TextRun({ text: '' })] })];
  }
  const childBlocks = [];
...
// ── top-level ────────────────────────────────────────────────────────────────
async function buildReportDocxV2({ geometry, slotMap, contentMap, textTransform, renderMode }) {
  const g = geometry || {};
  ...
  // for table cells, "p:block" for paragraph slots. Phase E + E-14: paragraph
  // slots with location.field_idx live at "p:block:fN" — a multi-field
  // demographic line packing several "Label: value" pairs into one paragraph
  // (e.g. "Case Name: ...  Date of report: ..."), where each pair is one slot
  // with its own field_idx. fieldSlotsByBlock indexes those slots per block in
  // field_idx order so paragraphFor can render the whole paragraph from N
  // slots at once. contentMap maps slot_id -> value. With no slotMap/
  // contentMap this stays a pure verbatim mirror (wk2 behaviour).
  const slotByLoc = {};
  const fieldSlotsByBlock = {}; // block_idx -> sorted [{field_idx, slot}, ...]
  for (const s of (slotMap && slotMap.slots) || []) {
    const L = s.location || {};
    if (L.row != null && L.cell != null) {
      slotByLoc[`${L.block}:${L.row}:${L.cell}`] = s;
    } else if (L.field_idx != null && L.block != null) {
      slotByLoc[`p:${L.block}:f${L.field_idx}`] = s;
      if (!fieldSlotsByBlock[L.block]) fieldSlotsByBlock[L.block] = [];
      fieldSlotsByBlock[L.block].push({ field_idx: L.field_idx, slot: s });
    } else if (L.block != null) {
      slotByLoc[`p:${L.block}`] = s;
    }
  }
  for (const b of Object.keys(fieldSlotsByBlock)) {
    fieldSlotsByBlock[b].sort((x, y) => x.field_idx - y.field_idx);
  }
  const repeatable = (slotMap && slotMap.repeatable_table) || null;

  // Phase D wk3-PT — static-scaffolding lookup + the render-mode SAFETY BOUNDARY.
  // content_fill (default): ONLY static scaffolding + filled slots emit source
  // text; unclassified locations blank — the source template's clinical content
  // is off-limits. verbatim is opt-in (Mirror debug screen / fidelity tests).
  const staticByLoc = {};
  for (const st of (slotMap && slotMap.static_text) || []) {
    const L = st.location || {};
    if (L.row != null && L.cell != null) staticByLoc[`${L.block}:${L.row}:${L.cell}`] = true;
    else if (L.block != null) staticByLoc[`p:${L.block}`] = true;
  }
  const mode = renderMode === 'verbatim' ? 'verbatim' : 'content_fill';

  const { config: numConfig, refs: numRefs } = buildNumbering(g.numbering_definitions);
  const ctx = { mediaByRel, numRefs, textTransform, slotByLoc, staticByLoc, fieldSlotsByBlock, contentMap: contentMap || null, repeatable, renderMode: mode };
  ...
  // Mirror P1 — header/footer bands (PDF source only; empty for the .docx path
  // and for clean reports with no recurring band). Render the band image in the
  // Word HEADER/FOOTER so it repeats per page at a fixed position, and widen the
  // body's top/bottom margins so text clears the band. CONDITIONAL: with no band
  // media, nothing here fires and the section is byte-for-byte the prior output.
  const headerMedia = (g.header_media || []).filter((m) => m && m.bytes);
  const footerMedia = (g.footer_media || []).filter((m) => m && m.bytes);
  const emuToDxa = (emu) => Math.round((emu || 0) / EMU_PER_DXA);
  const BAND_GAP_DXA = 144; // ~7.2pt clearance between a band and body text
  const pageMargin = {
  ...
  // Fit bands to the A4 PRINTABLE width (page minus left/right margins) so a
  // band wider than the page never clips at the right edge. Aspect preserved;
  // the margin clearance uses the SCALED height so the gap stays tight.
  const printableWidthEmu = Math.max(1, (pageSize.width - pageMargin.left - pageMargin.right)) * EMU_PER_DXA;
  const headerScaled = headerMedia.map((m) => fitMediaToWidth(m, printableWidthEmu));
  const footerScaled = footerMedia.map((m) => fitMediaToWidth(m, printableWidthEmu));
  if (headerScaled.length) {
    pageMargin.header = 0; // band flush to the top edge
    pageMargin.top = Math.max(pageMargin.top, emuToDxa(headerScaled[0].anchor && headerScaled[0].anchor.height) + BAND_GAP_DXA);
  }
  if (footerScaled.length) {
    pageMargin.footer = 0; // band flush to the bottom edge
    pageMargin.bottom = Math.max(pageMargin.bottom, emuToDxa(footerScaled[0].anchor && footerScaled[0].anchor.height) + BAND_GAP_DXA);
  }
  ...
  if (headerScaled.length) section.headers = { default: new Header({ children: [bandParagraph(headerScaled)] }) };
  if (footerScaled.length) section.footers = { default: new Footer({ children: [bandParagraph(footerScaled)] }) };
...
module.exports = { buildReportDocxV2, EMU_PER_PX, EMU_PER_DXA, _internals: { buildCellModel, buildNumbering, fitMediaToWidth } };
